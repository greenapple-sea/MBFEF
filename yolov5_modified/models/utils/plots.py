...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                # 计算标签框位置（添加基线偏移补偿）
                outside = box[1] - (h + baseline_offset) >= 0
                label_position = [
                    box[0],
                    box[1] - h - baseline_offset if outside else box[1],
                    box[0] + w + 1,
                    (box[1] + 1) if outside else (box[1] + h + 1)
                ]

                # 绘制标签背景框
                self.draw.rectangle(label_position, fill=color)

                # 计算文本绘制位置（添加垂直偏移补偿）
                text_y = box[1] - h - baseline_offset if outside else box[1]
                self.draw.text(
                    (box[0], text_y),
                    label,
                    fill=txt_color,
                    font=self.font,
                    anchor='lb'  # 明确锚点为左下角(left-bottom)
                )
        else:  # cv2
            p1, p2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
            cv2.rectangle(self.im, p1, p2, color, thickness=self.lw, lineType=cv2.LINE_AA)
    ...
    # 修改后的Annotator.text方法
    def text(self, xy, text, txt_color=(255, 255, 255)):
        """添加兼容Pillow 10.0+的文本尺寸获取方案"""
        try:
            # 新版Pillow（>=10.0.0）使用getbbox
            left, top, right, bottom = self.font.getbbox(text)
            w, h = right - left, bottom - top
        except AttributeError:
            # 旧版Pillow（<10.0.0）保持原逻辑
            w, h = self.font.getsize(text)

        # 调整坐标定位逻辑（解决新版垂直偏移问题）
        text_y = xy[1] - h + 1
        if hasattr(ImageFont, 'getbbox'):  # 仅在新版需要额外偏移补偿
            text_y += 2  # 经验性调整值

        # 绘制文本（添加防越界保护）
        self.draw.text(
            (xy[0], max(0, text_y)),  # 确保y坐标不小于0
            text,
            fill=txt_color,
            font=self.font,
            anchor='lb'  # 明确锚点位置（left-bottom）
        )
        return self
